var TOTAL_MONEY;
var START_MONEY;
var WIN_OCCURRENCE;         //WIN PERCENTAGE. SET A VALUE FROM 0 TO 100.
var FREESPIN_OCCURRENCE;    //FREESPIN OCCURRENCE IF THERE IS A WINNING COMBO
var BONUS_OCCURRENCE;       //BONUS OCCURRENCE IF THERE IS A WINNING COMBO
var SLOT_CASH;             //THIS IS THE CURRENT SLOT CASH AMOUNT. THE GAME CHECKS IF THERE IS AVAILABLE CASH FOR WINNINGS.

var NUM_FREESPIN;     //THIS IS THE NUMBER OF FREESPINS IF IN THE FINAL WHEEL THERE ARE 3,4 OR 5 FREESPIN SYMBOLS
var BONUS_PRIZE; //THIS IS THE LIST OF BONUS MULTIPLIERS.
var BONUS_PRIZE_OCCURRENCE;
var MAX_PRIZES_BONUS;     //MAX NUMBR OF PRIZES ASSIGNED IN BONUS GAME
...
var FREESPIN_NUM_OCCUR;
var _aPaylineCombo = new Array();
var _aFinalSymbols;

    
function APIgetSlotInfos(oCallback, oCallbackOwner){
    oCallback.call(oCallbackOwner,{start_money:TOTAL_MONEY,bets:COIN_BETS,start_bet:COIN_BETS[0],paytable:PAYTABLE_VALUES});
}
...
function apiAttemptBonus(iCoin,oCallback, oCallbackOwner){
    ...
    if(aPrizeList.length === 0){
        aPrizeList = [0];
        iTotWin = 0;
    }else if(aPrizeList.length > 1 && iMaxWin === aPrizeList[aPrizeList.length-1]){
        bFinalPrize = true;
    }

    var oData = {res:true,money:TOTAL_MONEY,bonus_win:iTotWin,prize_list:JSON.stringify(aPrizeList),final_prize:bFinalPrize};     
...
function generateRandomSymbols(bFreespin){
    ...
    if(bFreespin){
        ...
        var aTmp = new Array();
        for(i=0;i<FREESPIN_NUM_OCCUR.length;i++){
            for(j=0;j<FREESPIN_NUM_OCCUR[i];j++){
                aTmp.push(i);
            }
        }
        ...
        for(var j=0;j<NUM_REELS;j++){
            if(j == 0){
                ...
                if(iRandSymbol === BONUS_SYMBOL){
                    iNumBonus++;
                }else if(iRandSymbol === FREESPIN_SYMBOL){
                    iNumFreeSpins++;
                }
            }  
        }
