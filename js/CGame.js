function CGame(){
    ...
    this._init = function(){
        ...
        //ADD SUSPANCE ANIMS
        _aSuspanceReel = new Array();
        for(var t=1;t<NUM_REELS;t++){
            _aSuspanceReel[t] = new CSuspanceEffect(START_REEL_OFFSET_X + (t)*(SYMBOL_WIDTH + SPACE_BETWEEN_SYMBOLS),START_REEL_OFFSET_Y,_oContainerSlot);
        }
        
        var aSpritesLogo = new Array();
        for(var i=0;i<62;i++){
            aSpritesLogo.push(s_oSpriteLibrary.getSprite('logo_'+i));
        }
        ...
        _oLogo = createSprite(oSpriteSheet, "normal",256,0,512,209);
    ...
    this.stopNextReel = function() {
        ...
        if(_iNumReelsStopped%2 === 0){
            for(var k=1;k<NUM_REELS;k++){
                _aSuspanceReel[k].hide();
            }
        ...
        if(_iNextColToStop >1 && (_iCurNumBonusSymbolsInReels === 2 || _iCurNumFreespinSymbolsInReels === 2)){
            ...
            _aSuspanceReel[_iNextColToStop].show();
        }else{
    ...
    this.onBonusStart = function(oData){
        ...
        _oBonusPanel.show(JSON.parse(oData.prize_list),_iCurBet,oData.final_prize);
